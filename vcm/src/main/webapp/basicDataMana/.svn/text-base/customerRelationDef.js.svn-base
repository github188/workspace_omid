Ext.onReady(function() { 
   ...
   var importUserGridStore = Ext.create('Ext.data.Store', {
	...
	var consSortStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
		    type : 'ajax',
		    url : 'impCustomerManaAction!querySeaVwConsSortCode.action',
		    reader : {
		        root : 'consSortList'
 	        }
	    }
	...
	var tradeStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
		    type : 'ajax',
		    url : '',//impCustomerManaAction!querySeaVwTrade.action
		    reader : {
		        root : 'tradeList'
 	        }
	    }
	...
	var hecIndustryStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
		    type : 'ajax',
		    url : 'impCustomerManaAction!queryVwHecIndustryCode.action',
		    reader : {
	...
	var hecIndustryComboBox = Ext.create('Ext.form.field.ComboBox', {
	    ...
	    store : hecIndustryStore,
		editable : false
	});	
	
	Ext.define('isVipModel',{
		extend : 'Ext.data.Model',
		fields : [{
			type : 'string',
			name : 'IS_VIP_CODE'
	...
	var isVipStore = Ext.create('Ext.data.Store', {
		model : 'isVipModel',
		proxy : {
		    type : 'ajax',
		    url : 'impCustomerManaAction!queryVwIsVip.action',
		    reader : {
		        root : 'isVipList'
 	        }
	    }
	...
	var isVipComboBox = Ext.create('Ext.form.field.ComboBox', {
	    ...
	    width : 200,
	    labelAlign : 'right',
	    valueField: 'IS_VIP_CODE',
	    displayField: 'IS_VIP_NAME',
	    store : isVipStore,
		editable : false
	});	
		  ...
		  if(editConsNoField.getValue()!='' && editConsNameField.getValue()!=''){
			   ...
			   if(meterRow!=null && meterRow.length>0){
				   for(var i=0;i<meterRow.length;i++){
					   meterId = meterRow[i].get('METER_ID');
					   operator = meterRow[i].get('OPERATOR');
				       if(i!=0) meterIds = meterIds + ';' + meterId+','+operator;
				       else
				       meterIds = meterId+','+operator;
				   }
			   }
			   var userParams = {
	...
	Ext.define('meterGridModel', {
		extend : 'Ext.data.Model',
		fields : ['METER_ID','METER_NAME','DEV_ID','FAC_ID','FAC_NAME','OPERATOR']
	});
   ...
   var importVcmGridStore = Ext.create('Ext.data.Store', {
	...
	var vcmTypeStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
		    type : 'ajax',
		    url : 'impCustomerManaAction!queryVwCustType.action',
		    reader : {
		        root : 'vcmTypeList'
 	        }
	    }
	...
	var economyTypeStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
		    type : 'ajax',
		    url : 'impCustomerManaAction!queryVwEconomyType.action',
		    reader : {
	...
	var creditLevelStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
		    type : 'ajax',
		    url : 'impCustomerManaAction!queryVwCreditLevel.action',
		    reader : {
	...
	var valueLevelStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
		    type : 'ajax',
		    url : 'impCustomerManaAction!queryVwValueLevel.action',
		    reader : {
		        root : 'valueLevelList'
 	        }
	    }
	...
	var riskLevelStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
		    type : 'ajax',
		    url : 'impCustomerManaAction!queryVwRiskLevel.action',
		    reader : {
		        root : 'riskLevelList'
 	        }
	    }
	...
	var vipFlagStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
		    type : 'ajax',
		    url : 'impCustomerManaAction!queryVwVipFlag.action',
		    reader : {
		        root : 'vipFlagList'
 	        }
	    }
	...
	var industryStore = Ext.create('Ext.data.Store', {
		...
		proxy : {
		    type : 'ajax',
		    url : 'impCustomerManaAction!queryVwIndustryCode.action',
		    reader : {
		        root : 'industryList'
 	        }
	    }
